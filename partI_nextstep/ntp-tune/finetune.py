...
def parse_args():
    ...
    parser.add_argument(
        "--valid_file", type=str, default=None, help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
    ...
    parser.add_argument(
        '--allow_tokenizer_modification',
        action='store_true',
        help='Allow tokenizer modification when set, including adding special tokens and hence resizing embeddings.',
    )
...
def main():
    ...
    if args.allow_tokenizer_modification:
        ...
        if isinstance(tokenizer, LlamaTokenizer) or isinstance(tokenizer, LlamaTokenizerFast):
            ...
            # In some cases, the model's embedding matrix is of a greater dimension
            # that the size of its associated tokenizer. This discrepancy leads to a
            # VLLM error. E.g. https://github.com/vllm-project/vllm/issues/516.
            # As a workaround, we add extra tokens to the tokenizer so that its size
            # equals the size of the model's embedding matrix. 
            num_embeddings = model.get_input_embeddings().weight.shape[0]
            tokenizer.add_tokens(['[EXTRA__%d]' % i for i, _ in enumerate(
                range(num_embeddings-len(tokenizer))
            )])
    ...
    if "prompt" in raw_datasets["train"].column_names and "completion" in raw_datasets["train"].column_names:
        ...
        raise ValueError("You need to have in your column names.")
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers("finetune", experiment_config)
            ...
            if accelerator.sync_gradients:
                ...
                if isinstance(checkpointing_steps, int):
                    if completed_steps % checkpointing_steps == 0:
                        ...
                        # Evaluate validation loss if a valid dataset is present.
                        if args.valid_file:
                            model.eval()
                            total_valid_loss = 0
                            n_valid_batches = 0
                            if accelerator.is_main_process:
                                logger.info(f"  --- Validation Start --- Step: {completed_steps}")
                            for batch in tqdm(valid_dataloader, total=len(valid_dataloader), disable=not accelerator.is_main_process):
                                batch.to(accelerator.device)
                                with torch.no_grad():
                                    outputs = model(**batch, use_cache=False)
                                    total_valid_loss += outputs.loss.detach().float()
                                    n_valid_batches += 1
                            total_valid_loss = accelerator.gather(total_valid_loss).sum().float()
                            valid_loss = total_valid_loss /  (n_valid_batches*accelerator.num_processes)
                            if accelerator.is_main_process:
                                ...
                                logger.info(f"  --- Validation End --- Step: {completed_steps} Loss: {valid_loss} Best: {best_valid_loss} Best step: {best_valid_step}")
                            model.train()
